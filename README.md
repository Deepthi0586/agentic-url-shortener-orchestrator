...
A prototype demonstrating agentic SDLC orchestration: an orchestration
engine that takes a software requirement, decomposes it into tasks,
executes them through a stateful, gated workflow, and produces a
reviewable engineering outcome — demonstrated by using it to build a
...
- **`shortener-service/`** — the URL shortener API, analytics, and reliability
  features, generated/modified by the orchestrator across the scenarios.
...
See `scenarios/` for saved logs of all scenarios already run (greenfield,
brownfield, ambiguous, and the JWT authentication extension),
...
(greenfield, brownfield, ambiguous) have been run end-to-end, plus an
additional brownfield extension adding JWT authentication.

## Extensions

The orchestrator was used to add JWT-based bearer token authentication
to the URL shortener's management endpoints (create, retrieve
metadata, delete), while keeping the public redirect endpoint
unauthenticated. Codebase Reasoning analyzed the existing service,
Design proposed the API contract and data model, Implementation
generated the Spring Security and JJWT integration, and Testing
generated and ran 12 real tests, all passing.
